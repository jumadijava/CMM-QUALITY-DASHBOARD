...
class DashboardPage:
    ...
    def _count_ng_parts(self, df: pd.DataFrame) -> int:
        if df.empty:
            return 0
        df_prod = df[df["Category"] == "Produksi"].copy() if "Category" in df.columns else df.copy()
        if df_prod.empty:
            return 0
        df_prod["_date"] = df_prod["Date"].dt.date
        group_cols = ["_date", "Shift", "Cycle", "PartName", "ModelName"]
        if "SampleNo" in df_prod.columns:
            group_cols.insert(3, "SampleNo")
        has_ng = df_prod.groupby(group_cols)["Judgement"].apply(lambda x: (x == "NG").any())
        return int(has_ng.sum())
    ...
    def render(self):
        ...
        with hcol1:
            st.markdown(f"""
            <div class="page-hdr" style="display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid #E8ECF2;padding-bottom:12px;margin-bottom:20px;">
        ...
        with hcol2:
            st.markdown
            if st.button("🔄", key="dash_refresh", help="Refresh data"):
                # Clear semua cache
                st.cache_data.clear()
    ...
    def _render_main(self, filters):
        ...
        f_part  = filters.get("part", "All Part")
        f_model = filters.get("model", "All Model")
        f_d1 = filters["d1"]
        f_d2 = filters["d2"]

        # ── Apply filter CMM dari filter utama ───────────────────
        _f_cmm = filters.get("cmm", "All CMM")
        if _f_cmm and _f_cmm not in ("All CMM", "All", "Semua", None, ""):
        ...
        _f_cmm_alert = filters.get("cmm", "All CMM")
        _alerts = self._get_prediction_alerts(
            ...
            filter_cmm   = _f_cmm_alert if _f_cmm_alert not in ("All CMM","All","Semua",None,"") else None,
        )
        if _alerts:
            slides_html = ""
        ...
        with c1:
            st.markdown(f'<div class="kpi-card"><div class="kpi-icon kpi-blue">{self.svg_check()}</div><div><div class="kpi-val">{TOTAL_CHECKED:,}</div><div class="kpi-lbl">Total Points Checked</div></div></div>', unsafe_allow_html=True)
        with c2:
            st.markdown(f'<div class="kpi-card"><div class="kpi-icon kpi-blue" style="background:#F5F3FF;"><svg viewBox="0 0 24 24" fill="none" stroke="#7C3AED" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="7" width="20" height="14" rx="2"/><path d="M16 3H8"/></svg></div><div><div class="kpi-val" style="color:#7C3AED;">{total_part:,}</div><div class="kpi-lbl">Total Parts Measured</div></div></div>', unsafe_allow_html=True)
        with c3:
            st.markdown(f'<div class="kpi-card"><div class="kpi-icon kpi-red" style="background:#FEF2F2;"><svg viewBox="0 0 24 24" fill="none" stroke="#DC2626" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="7" width="20" height="14" rx="2"/><path d="M16 3H8"/><line x1="9" y1="14" x2="15" y2="14"/></svg></div><div><div class="kpi-val kpi-val-red">{ng_parts:,}</div><div class="kpi-lbl">Parts NG</div></div></div>', unsafe_allow_html=True)
        with c4:
            st.markdown(f'<div class="kpi-card"><div class="kpi-icon kpi-green">{self.svg_percent()}</div><div><div class="kpi-val kpi-val-green">{OK_RATIO}%</div><div class="kpi-lbl">Average OK Ratio</div></div></div>', unsafe_allow_html=True)
        with c5:
            st.markdown(f'<div class="kpi-card"><div class="kpi-icon kpi-red">{self.svg_cross()}</div><div><div class="kpi-val kpi-val-red">{NG_COUNT:,}</div><div class="kpi-lbl">Total Titik NG</div></div></div>', unsafe_allow_html=True)
        ...
        _cmm_df = self.df_all
        if f_part not in ("All Part", "All", "Semua Part", None, ""):
            _cmm_df = _cmm_df[_cmm_df["PartName"] == f_part]
        cmm_names = sorted(_cmm_df["CMMName"].dropna().unique().tolist())
        _sc = filters.get("cmm", "All CMM")
        if _sc and _sc not in ("All CMM", "All", "Semua", None, ""):
                ...
                <div style="background:white;border-radius:8px;border:1px solid #E2E8F0;
                  ...
                  <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;">
                    <div style="display:flex;align-items:center;gap:8px;">
                      <span style="width:9px;height:9px;border-radius:50%;background:{st_dot};
                            box-shadow:0 0 6px {st_dot};display:inline-block;"></span>
                      <span style="font-size:15px;font-weight:700;color:#0F172A;">{cmm_name}</span>
                    </div>
                    <span style="font-size:10px;font-weight:700;color:{st_color};
                          background:{st_bg};padding:3px 10px;border-radius:10px;">{st_text}</span>
                  </div>
                  <div style="display:flex;justify-content:space-between;align-items:center;
                    ...
                    <div>
                      <div style="font-size:10px;color:#94A3B8;font-weight:600;text-transform:uppercase;
                           letter-spacing:.4px;">Last Measurement</div>
                      <div style="font-size:12px;font-weight:700;color:#0F172A;margin-top:2px;">
                        {last_part} · {last_sample}
                      </div>
                      <div style="font-size:10px;color:#64748B;margin-top:1px;">{last_str} &nbsp;·&nbsp; {idle_str}</div>
                    </div>
                    <span style="font-size:11px;font-weight:800;color:{j_color};
                          background:{j_bg};padding:4px 10px;border-radius:8px;">{last_judgement}</span>
                  </div>
                  <div style="font-size:10px;color:#94A3B8;font-weight:600;text-transform:uppercase;
                       letter-spacing:.4px;margin-bottom:6px;">Periode Filter</div>
                  <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr 1fr;gap:6px;">
                    <div style="text-align:center;padding:8px 4px;background:#F8FAFC;border-radius:6px;">
                      <div style="font-size:16px;font-weight:800;color:#0F172A;">{shift_ok_pct}%</div>
                      <div style="font-size:9px;color:#64748B;font-weight:600;margin-top:1px;">OK Rate</div>
                    </div>
                    <div style="text-align:center;padding:8px 4px;background:#EFF6FF;border-radius:6px;">
                      <div style="font-size:16px;font-weight:800;color:#2563EB;">{parts_ukur:,}</div>
                      <div style="font-size:9px;color:#64748B;font-weight:600;margin-top:1px;">Parts</div>
                    </div>
                    <div style="text-align:center;padding:8px 4px;background:#FEF2F2;border-radius:6px;">
                      <div style="font-size:16px;font-weight:800;color:#DC2626;">{parts_ng_cmm:,}</div>
                      <div style="font-size:9px;color:#64748B;font-weight:600;margin-top:1px;">Parts NG</div>
                    </div>
                    <div style="text-align:center;padding:8px 4px;background:#F0FDF4;border-radius:6px;">
                      <div style="font-size:16px;font-weight:800;color:#16A34A;">{shift_ok:,}</div>
                      <div style="font-size:9px;color:#64748B;font-weight:600;margin-top:1px;">OK Pts</div>
                    </div>
                    <div style="text-align:center;padding:8px 4px;background:#FEF2F2;border-radius:6px;">
                      <div style="font-size:16px;font-weight:800;color:#DC2626;">{shift_ng:,}</div>
                      <div style="font-size:9px;color:#64748B;font-weight:600;margin-top:1px;">NG Pts</div>
                    </div>
                  </div>
                  {kp_ng_str}
                </div>
        ...
        SAMPLE_TARGETS = {
            ...
            ("CRCS L",      "K2SA",    "0"):  4,
            ("CRCS R",      "K2SA",    "0"):  8,
            ("MISSION CASE","K60",     "0"):  5,
        }

        def _get_target(part, model, shift):
            """Ambil target dengan fallback ke shift "0" (default)."""
            s = str(shift)
            return (SAMPLE_TARGETS.get((part, model, s)) or
                    SAMPLE_TARGETS.get((part, model, "0")) or 0)
        ...
        if df.empty:
            ...
            parts_in_data = sorted(df["PartName"].dropna().unique().tolist())

            for part in parts_in_data:
                df_part = df[df["PartName"] == part]
                models  = sorted(df_part["ModelName"].dropna().unique().tolist())
                    ...
                    with cols[ci]:
                        ...
                        <div style="background:white;border-radius:8px;border:1px solid #E2E8F0;
                          ...
                          <div style="display:flex;justify-content:space-between;align-items:start;margin-bottom:10px;">
                            <div style="font-size:13px;font-weight:700;color:#0F172A;">{model}{kp_badge}</div>
                            <span style="font-size:10px;font-weight:700;color:{status_color};background:{status_bg};
                                  padding:3px 8px;border-radius:10px;white-space:nowrap;">{status_text}</span>
                          </div>
                          ...
                          <div style="display:flex;gap:16px;padding-top:10px;border-top:1px solid #F1F5F9;">
                            <div>
                              <div style="font-size:14px;font-weight:700;color:#22C55E;">{ok:,}</div>
                              <div style="font-size:9px;color:#64748B;font-weight:600;">OK</div>
                            </div>
                            <div>
                              <div style="font-size:14px;font-weight:700;color:#EF4444;">{ng:,}</div>
                              <div style="font-size:9px;color:#64748B;font-weight:600;">NG</div>
                            </div>
                            <div>
                              <div style="font-size:14px;font-weight:700;color:#0F172A;">{tot:,}</div>
                              <div style="font-size:9px;color:#64748B;font-weight:600;">Total</div>
                            </div>
                            <div style="margin-left:auto;text-align:right;">
                              <div style="font-size:14px;font-weight:700;color:#DC2626;">{ng_parts_m:,}</div>
                              <div style="font-size:9px;color:#64748B;font-weight:600;">Parts NG</div>
                            </div>
                          </div>
                          {target_html}
                        </div>
        ...
        # ── KPI per SampleNo + Status CMM (muncul kalau filter Part dipilih) ──
        if f_part not in ("All Part", "All", "Semua Part", None, "") and not df.empty:

            # ── Kualitas per Sample ───────────────────────────────────
            st.markdown(
                f'<div style="font-size:14px;font-weight:700;color:#0F172A;'
                f'margin-bottom:10px;">Kualitas per Sample — {f_part}</div>',
                unsafe_allow_html=True
            )
            # Sort numerik: "1","2"..."9","10","11"... bukan string sort
            raw_samples = df["SampleNo"].dropna().astype(str).unique().tolist()
            def _sno_key(s):
                try: return (0, int(s))
        ...
        # ── Toggle mode tren ────────────────────────────────────────
        part_selected = f_part not in ("All Part", "All", "Semua Part", None, "")
        ...
        if delta_days == 0:
            ...
            if f_model not in ("All Model", "All", "Semua Model", None, ""):
            ...
            if df_trend.empty or df_ctx2.empty:
                ...
                if trend_mode in ("Per SampleNo", "Per Part & Model"):
                    ...
                    if part_selected and trend_mode == "Per SampleNo":
                        ...
                        df_ctx2_f = df_ctx2[df_ctx2["PartName"] == f_part].copy() if f_part not in ("All Part","") else df_ctx2.copy()
        ...
        # ─────────────────────────────────────────────────────────────
        st.markdown('<div style="font-size:16px;font-weight:700;color:#0F172A;margin-top:28px;margin-bottom:12px;">⏱️ Latest Incoming Data</div>', unsafe_allow_html=True)
