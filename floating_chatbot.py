...
def _get_hybrid_response(user_text: str) -> str:
    ...
    if df.empty:
        return "Maaf, saya tidak bisa menemukan atau membaca data di `full.csv`."

    max_rows = 8000
    if len(df) > max_rows:
        df_context = df.tail(max_rows)
        warning_msg = f"Catatan: Saya hanya menganalisis {max_rows} baris terakhir agar tetap cepat."
    else:
        df_context = df
        warning_msg = ""

    csv_string = df_context.to_csv(index=False)

    # ── PROMPT BARU YANG LEBIH LUWES ──
    system_prompt = f"""
    ...
    1. JIKA PENGGUNA MENYAPA (misal: "pagi", "halo", "bro", "terima kasih"): 
       ...
       - Jangan bertingkah kaku atau menyebutkan bahwa kamu sedang membaca CSV.
    ...
    with st.popover("💬 Tanya AI"):
        ...
        with chat_box:
            ...
            if st.session_state.chatbot_history and st.session_state.chatbot_history[-1]["role"] == "user":
                # ── TEKS LOADING JUGA DIUBAH AGAR LEBIH NATURAL ──
