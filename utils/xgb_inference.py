...
def run_xgb_inference(df_all: pd.DataFrame) -> pd.DataFrame | None:
    """
    ...
    # Return dari cache kalau masih valid
    if (cache_key in st.session_state and
            time.time() - st.session_state.get(ts_key, 0) < TTL_SECONDS):
...
def get_prediction_alerts(df_all: pd.DataFrame,
    ...
    """
    result = run_xgb_inference(df_all)
...
def run_rule_prediction(df_all: pd.DataFrame,
    ...
    if (RULE_CACHE_KEY in st.session_state and
            time.time() - st.session_state.get(ts_key, 0) < RULE_TTL):
...
def get_rule_alerts(df_all: pd.DataFrame,
    ...
    """
    rows = run_rule_prediction(df_all)
    if not rows:
        return []
